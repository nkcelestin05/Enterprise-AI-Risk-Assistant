# =============================================================================
# Enterprise AI Risk Assessment Assistant - AWS Infrastructure
# Provisions: VPC, ECR, ECS Fargate, ALB, RDS PostgreSQL, Security Groups
# =============================================================================
...
terraform {
  ...
  backend "s3" {
    bucket         = "risk-assistant-terraform-state"
...
resource "aws_ecs_task_definition" "app" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      { name = "DATABASE_URL",    value = "postgresql://${var.db_username}:${var.db_password}@${aws_rds_cluster.main.endpoint}:5432/${var.db_name}" },
      { name = "NEXTAUTH_URL",    value = "https://${var.domain_name}" },
      { name = "NODE_ENV",        value = "production" }
    ]
    secrets = [
      { name = "NEXTAUTH_SECRET",  valueFrom = "${aws_ssm_parameter.nextauth_secret.arn}" },
      { name = "ABACUSAI_API_KEY", valueFrom = "${aws_ssm_parameter.api_key.arn}" }
    ]
    logConfiguration = {
      logDriver = "awslogs"
      options = {
...
resource "aws_ssm_parameter" "api_key" {
  name  = "/${var.app_name}/${var.environment}/ABACUSAI_API_KEY"
  type  = "SecureString"
  value = var.abacusai_api_key
}
